...
test_img = phi(gblur, p_true, w_true) #.+ rand(32,32,15)/1000#rand(1024)./1000
...
function trial_gradient(test_img, _trial_point, _trial_weight)
    ...
    trial_weight = [_trial_weight]
    residuals = resids(test_img, trial_point, trial_weight)
    gradient = SparseInverseProblems.computeGradient(gblur, trial_weight, trial_point, residuals)
    return gradient
end
